              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    SizedBox(
                        width: 360.0,
                        child: !_hasCode ? _passwordResetEmailForm() : PasswordResetPasswordFormController()
                    ),
                    const SizedBox(height: 20.0),
                    if (authVM.state == AuthViewState.busy) LoadWidget() else const SizedBox(),
                    const SizedBox(height: 40.0),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: <Widget>[
                        if (_hasCode) TextButton(
...
//                  ),
                    ...
                    onFieldSubmitted: (_) => submitEmail(authVM),
                  ...
                  Wrap(
                    children: [
                      const Text("Enter email for the account you want to reset the password for and a recovery code will be sent to that email", style: kTextStyleSmallSecondary,),
                    ],
                  ),
                  const SizedBox(height: 50.0),
                  ElevatedButton( //style: TextStyle(color: TILE_DIV_LINE_COLOR)
                    onPressed: () => submitEmail(authVM),
  ...
  Future<void> submitEmail(AuthViewModel authVM) async {
...
class PasswordResetPasswordFormController extends StatefulWidget {
  @override
  _PasswordResetPasswordFormControllerState createState() => _PasswordResetPasswordFormControllerState();
}
class _PasswordResetPasswordFormControllerState extends State<PasswordResetPasswordFormController> {
  final _formKey = GlobalKey<FormState>();
          ...
          builder: (context, watch, child) {
            ...
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: <Widget>[
                  const Text("Enter Code Below", style: kTextStyleSubHeading,),
...
//                    color: TEXT_COLOR,
                      ...
                      suffixIcon: GestureDetector(
                        onTap: () async => setState(() => _hideCheckPassword = !_hideCheckPassword),
                        child: Icon(
                          _hideCheckPassword ? Icons.visibility_outlined : Icons.visibility_off_outlined,
                        ),
                      ),
                    ),
                    onFieldSubmitted: (_) => submit(authVM),
  ...
  Future<void> submit(AuthViewModel authVM) async {
    final code = _pinPutController.text.trim();
    if (code.isEmpty || code.length < 6) InfoSnackBar.show(context, "Please enter the code sent to your email", color: SnackBarColor.error);
    if (_formKey.currentState!.validate()) {
      await authVM.passwordReset(_passwordController.text.trim(), _pinPutController.text.trim());
    }
