<?php
	...
	// Delete all reset tokens that are inactive
	$dateTimeNow = gmdate('Y-m-d H:i:s', time());
	// Delete all inactive refreshTokens
	$result = $db->query("DELETE FROM passwordResetToken WHERE activeTill<'$dateTimeNow'");
	...
	$result = $db->query("SELECT ID FROM $dbTableNameUser WHERE email='$email' LIMIT 1");
	...
	$dateTimeNow = gmdate('Y-m-d H:i:s',strtotime('+15 minutes',strtotime($dateTimeNow)));


	// add refreshToken to db
	$result = $db->query("
		INSERT INTO passwordResetToken (code, email, activeTill)
		VALUES ('', '$email', '$dateTimeNow')
	");
	if (!$result) {
		http_response_code(503);
		//echo mysqli_error($db);
		echo '{"Message":"Error storing reset token"}';
		return;
	}
	...
	// Create unique code
	$uniqueCodeAdded = false;
	$retries = 21;
	while (!$uniqueCodeAdded && $retries > 0) {
		$code = sprintf("%06d", mt_rand(1, 999999));

		// check if code doesnt already exists
		$result = $db->query("SELECT ID FROM passwordResetToken WHERE code='$code'");
		if ($result->num_rows == 0) {
			$uniqueCodeAdded = true;

			// Add unique code to resetToken
			$result = $db->query("UPDATE passwordResetToken SET code='$code' WHERE ID='$lastId'");
	...
	// Get resetToken and unique code
	$result = $db->query("SELECT code FROM passwordResetToken WHERE ID='$lastId'");
	if ($result->num_rows == 0) {
		http_response_code(503);
		//echo mysqli_error($db);
		echo '{"Message":"Token Find Error"}';
		return;
	}
										...
										<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
											<tr>
												<td class="text" style="text-align: center; padding: 20px 30px;">
													...
													<h1 style="color: white;">'.$resetCode.'</h1>
												</td>
											</tr>
										</table>
									</td>
								...
								<tr>
									<td class="bg_white email-section">
										<div class="heading-section" style="text-align: center; padding: 0 30px;">
											<p>If you did not request a password reset.<br>Please make sure no one else is attempting to access your account</p>
									</td>
								</tr>
								<tr>
