<?php
	...
	$stmt = $db->prepare("SELECT userUuid FROM userNotify WHERE userUuid=? AND storeUuid=?");
	...
	if ($foundExisting) {
		$stmt = $db->prepare("
			UPDATE userNotify SET status=?
			WHERE userUuid=? AND storeUuid=?
		");
	} else {
			$stmt = $db->prepare("
			INSERT INTO userNotify (status, userUuid, storeUuid)
			VALUES (?, ?, ?)
		");
