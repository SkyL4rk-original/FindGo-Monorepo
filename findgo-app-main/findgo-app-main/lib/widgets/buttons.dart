...
class ToggleStoreFollowButton extends StatefulWidget {
  ...
  const ToggleStoreFollowButton({Key? key, required this.storesVM, required this.store}) : super(key: key);

  @override
  _ToggleStoreFollowButtonState createState() => _ToggleStoreFollowButtonState();
}
class _ToggleStoreFollowButtonState extends State<ToggleStoreFollowButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final followed = _storesVM.followedStoresUuidList.any((uuid) => uuid == _store.uuid);
    late Widget child;
    ...
    if (_busy) {
      ...
      child = const SizedBox(width: kButtonFollowingWidth,child: Text("Unfollow", textAlign: TextAlign.center,));
    } else {
      child = const SizedBox(width: kButtonFollowingWidth,child: Text("Follow", textAlign: TextAlign.center,));
    }
...
class ToggleStoreNotifyButton extends StatefulWidget {
  ...
  const ToggleStoreNotifyButton({Key? key, required this.storesVM, required this.store}) : super(key: key);

  @override
  _ToggleStoreNotifyButtonState createState() => _ToggleStoreNotifyButtonState();
}
class _ToggleStoreNotifyButtonState extends State<ToggleStoreNotifyButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final notify = _storesVM.notifyStoresUuidList.any((uuid) => uuid == _store.uuid);
    late Widget child;
    ...
    if (_busy) {
      ...
      child = const SizedBox(width: kButtonNotifyWidth, child: Icon(Icons.notifications_none, size: 20));
    } else {
      child = const SizedBox(width: kButtonNotifyWidth, child: Icon(Icons.notifications_active_outlined, size: 20));
    }
